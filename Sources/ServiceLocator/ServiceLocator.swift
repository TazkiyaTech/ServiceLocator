...
public class ServiceLocator {
    ...
    /// - Throws: ``ServiceLocatorError`` if the given module attempts to retrieve a service that does not exist in this service locator or attempts to register a service with a type that already exists.
    public func addModule(_ module: Module) throws(ServiceLocatorError) {
        try module.registerServices(in: self)
    }
    
    /// Adds the given service to this service locator. Registers the given service against its dynamic type.
    ///
    ...
    /// Adds the given service to this service locator. Registers the given service against the specified type.
    ///
    /// - Parameter service: The service to add to this service locator.
    /// - Parameter serviceType: The type against which the service will be added and later looked up.
    /// - Throws: ``ServiceLocatorError`` if a service is already registered in this service locator with the specified type.
    public func addService<T>(_ service: T, as serviceType: T.Type) throws(ServiceLocatorError) {
